...
- **Gender Hint**: Optional constraint on slot using Stashapp genders (MALE, FEMALE, TRANSGENDER_FEMALE, TRANSGENDER_MALE, NON_BINARY)
...
- [ ] Update database schema for slotted markers
  - [ ] Update `slot_definitions` table to use slot_label instead of slot_name
  - [ ] Add gender_hint field (enum or varchar referencing Stashapp genders)
  - [ ] Remove cardinality field (not needed)
  - [ ] Ensure stashapp_tag_id references primary tags
- [ ] Build slot definitions CRUD API
  - [ ] GET /api/slot-definitions - list all slot definitions
  - [ ] GET /api/slot-definitions/:tagId - get slot definitions for a tag
  - [ ] POST /api/slot-definitions - create slot definition for a tag
  - [ ] PUT /api/slot-definitions/:id - update slot definition
  - [ ] DELETE /api/slot-definitions/:id - delete slot definition
- [ ] Build Settings UI for slot definitions
  - [ ] Add "Slotted Markers" section to Settings page
  - [ ] Tag selector to choose primary tag
  - [ ] Slot definition editor (add/remove/reorder slots)
  - [ ] Slot label input field
  - [ ] Gender hint dropdown (optional)
- [ ] Update marker creation/editing to support slots
  - [ ] Display slot fields when primary tag has slot definitions
  - [ ] Performer picker for each slot
  - [ ] Filter performers by gender hint (if specified)
  - [ ] Allow empty slots
- [ ] Add slot display to marker views
  - [ ] Show assigned performers in marker list view
  - [ ] Show slot details in marker detail view
- [ ] Implement slot-based filtering
  - [ ] Filter markers by specific performer in specific slot
  - [ ] Filter markers by empty/filled slots
